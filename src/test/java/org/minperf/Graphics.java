...
public class Graphics {
  ...
  public static void generateSampleTikz() {
    ...
    System.out.println(
        "child[edge from parent=draw, line width=.1ex, sibling distance=30mm, level distance=0mm] {");
    bits.append(" & Header \\\\\n");
    ...
    for (int x = 0; x < bucketCount; x++) {
      ...
      System.out.println(
          "  child[" + getSizeTikz(bucketSize) + "] {child[level distance=8mm]{node {$b_" + x
              + "$}");
  ...
  private static void generateBitDescription(Settings settings, BitBuffer in, StringBuilder bits,
      int size) {
    if (size <= 1) {
      return;
    }
