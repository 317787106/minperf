...
public class SpaceEstimator {
      ...
      for (int averageBucketSize = 4; averageBucketSize < 4 * 1024; averageBucketSize *= 2) {
        ...
        for (int test = 0; test < testCount; test++) {
          ...
          RecSplitEvaluator<Long> eval =
              RecSplitBuilder.newInstance(hash).leafSize(leafSize)
  ...
  public static double getExpectedSpaceEstimate(int leafSize, int averageBucketSize) {
    ...
    double bitsPerKeyCalc =
        (totalBits + bitsPerBucketStartOverhead + bitsPerBucketOffsetOverhead) / averageBucketSize;
  ...
  public static double getExpectedBucketSpace(Settings s, int size, int indent,
      HashMap<Integer, Double> cache) {
    if (size <= 1) {
      return 0;
    }
...
//        if (split != split2) {
    if (split2 < 0) {
      System.out.println(
          "   size " + size + " split " + -split2 + ":" + (size + split2) + "; " + result);
    } else {
      System.out.println(
          "   size " + size + " split by " + split2 + "; " + result / size + " *** old:" + split);
    }
//        } else {
//            if (split2 < 0) {
...
//        }
  ...
  private static double getExpectedBucketSpace(Settings s, int size, int indent,
      HashMap<Integer, Double> cache, int split) {
  ...
  public static void listMaxRecursionDepth() {
    ...
    for (int averageBucketSize = 8; averageBucketSize < 100000; averageBucketSize *= 2) {
      ...
      System.out.println(
          "averageBucketSize=" + averageBucketSize + " max recursion depth: " + recursionDepth);
    }
  }
