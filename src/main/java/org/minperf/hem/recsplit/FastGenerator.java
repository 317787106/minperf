...
public class FastGenerator {
  ...
  private static void sortIntoBuckets(long[] keys, int len, int shift, int bucketCount,
      int[] offsetList, int[] array2) {
    if (bucketCount == 1) {
  ...
  private void generateBucket(long[] keys, int start, int end, int[] startList, int bucketId) {
    ...
    if (buff.position() > 0) {
      ...
      for (int overlap = 8; overlap > 0; overlap--) {
        if (buff.position() > overlap && bucketBuff.position() > overlap
            && buff.position() - lastStart >= overlap) {
